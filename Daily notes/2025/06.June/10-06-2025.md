...
Alright — time to leave toy code behind. You're stepping into **low-level generic data structures** now. Let’s rebuild your dynamic array (`Vector`) using `void*` and `size_t elem_size` so it works with any data type.
